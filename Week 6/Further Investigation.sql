...
SELECT 
    campaign_name
,   CASE WHEN Impression>0 THEN 'YES' ELSE 'NO' END AS Impression
,   SUM(CASE WHEN CartAdd>0 THEN 1 ELSE 0 END) AS CartAdd
,   COUNT(campaign_name) AS TotalCust
,   FORMAT(SUM(CASE WHEN CartAdd>0 THEN 1 ELSE 0 END)*1.00/COUNT(campaign_name),'P2') AS 'CartAdd to Total Customer Ratio'
FROM CTE_A
